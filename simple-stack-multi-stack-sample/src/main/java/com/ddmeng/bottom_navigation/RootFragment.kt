...
import com.ddmeng.bottom_navigation.databinding.RootFragmentBinding
import com.ddmeng.bottom_navigation.navigation.FragmentStackHostFragment
import com.ddmeng.bottom_navigation.utils.viewBinding
...
class RootFragment : KeyedFragment(R.layout.root_fragment) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            val firstFragment = FragmentStackHostFragment.newInstance(RootScreen.FIRST_STACK)
                .also { this.firstFragment = it }
            val secondFragment = FragmentStackHostFragment.newInstance(RootScreen.SECOND_STACK)
                .also { this.secondFragment = it }
            val thirdFragment = FragmentStackHostFragment.newInstance(RootScreen.THIRD_STACK)
                .also { this.thirdFragment = it }
            ...
            firstFragment =
                childFragmentManager.findFragmentByTag("firstFragment") as FragmentStackHostFragment
            secondFragment =
                childFragmentManager.findFragmentByTag("secondFragment") as FragmentStackHostFragment
            thirdFragment =
                childFragmentManager.findFragmentByTag("thirdFragment") as FragmentStackHostFragment
        }
    }
    ...
    fun addToCurrentStack(newKey: Any) {
        (fragments[selectedIndex] as FragmentStackHostFragment).stackHost.backstack.goTo(newKey)
    }
}